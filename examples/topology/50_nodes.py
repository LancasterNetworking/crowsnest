...
def sshd(network, cmd='/usr/sbin/sshd', opts='-D',
    ...
    for host in network.hosts:
        host.cmd(cmd + ' ' + opts + '&')
    network.getNodeByName('h1').cmd('sudo killall mongod > /dev/null 2>&1')
    network.getNodeByName('h1').cmd('sudo mkdir -p /data/db/')
    network.getNodeByName('h1').cmd('sudo rm -f /data/db/mongod.lock')
    network.getNodeByName('h1').cmd('mongod --bind_ip=10.0.0.1 --fork --logpath /var/log/mongo.log') # run mongod on h1
