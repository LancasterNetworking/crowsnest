...
def sshd( network, cmd='/usr/sbin/sshd', opts='-D',
    ...
    print network.getNodeByName('h1').cmd('sudo mkdir -p /data/db/')
    print network.getNodeByName('h1').cmd('sudo rm -f /data/db/mongod.lock')
    print network.getNodeByName('h1').cmd('mongod --bind_ip=10.0.0.1 --fork --logpath /var/log/mongo.log') # run mongod on h1
    print
